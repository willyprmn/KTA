...
class AuthController extends Controller
{
    ...
    public function updateProfil(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'password' => ['nullable'],
            'ktp' => ['required', 'regex:/[0-9]{9}/', Rule::unique('customers')->where(function ($query) use ($auth) {
                return $query->where('user_id', $auth);
            }), 'min:16', 'max:16'],
            'npwp' => ['required', 'regex:/[0-9]{9}/', 'min:15', 'max:15'],
            'jenis_kelamin' => ['required'],
            'status' => ['required'],
            'alamat' => ['required'],
            'no_hp' => ['required', 'regex:/[0-9]{9}/', Rule::unique('customers')->ignore(auth()->user()->id), 'min:10', 'max:13'],
            // 'rekening' => ['required'],
            'no_cc' => ['required', 'regex:/[0-9]{9}/', 'min:16', 'max:16'],
            'limit_cc' => ['required']
        ], [
